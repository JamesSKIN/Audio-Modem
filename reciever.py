...
def standard_deconstructor(aud, ofdm, channel_H = None, multi_est=False, retSymbs = False, ldpc_encoded = True, add_rotate=None):
    N = ofdm.N
    L = ofdm.CP
    QFSK_dict  = ofdm.QFSK_dict
    backwards_dict = {v : k for k, v in QFSK_dict.items()}
    ...
    if channel_H is None:
        pass
    elif multi_est == False:
        freq_data = freq_data / channel_H
    else:
        item=0
        for i in range(0,len(freq_data),60):
            try:
                print(i,item)
                freq_data[i:i+60] = freq_data[i:i+60] / channel_H[item]
                item+=1
            except:
                print(i,item,np.shape(freq_data))
                freq_data[i:] = freq_data[i:] / channel_H[item]
                item+=1
...
def twoSymbCE(two_known_symbols, ofdm):
    even_one = two_known_symbols[:ofdm.N+ofdm.CP]
    odd_one = two_known_symbols[ofdm.N+ofdm.CP:2*(ofdm.N+ofdm.CP)]
    odd_index = np.arange(0,741,2)
    even_index=np.arange(1,741,2)
    ...
    hest = np.zeros(741, dtype='complex')
    hest[even_index] = hes_e1[even_index]
    hest[odd_index] = hes_o1[odd_index]
...
def chirpEnds(signal, note=None, graph_display=False):
    ...
    peka = scipy.signal.find_peaks(norm_signal, 0.4)[0]
    if peka[0] <= 10000:
        start_search1 = 0
    else:
